#!/usr/bin/env python3
"""
train_gru_neopt_multiseed.py
GRU-NeOpt (GRUBackbone + SPO+) across 3 random seeds, own process.

Imports from step03_train_neopt.py (verified) and step03b_train_gru_neopt.py
(the GRUBackbone class was verified working earlier in this project, but
double check the import names below against your real file if this fails).

This is the model whose seed-robustness is most important to confirm,
GRU-NeOpt's single-run result was the most surprising finding in the
project. All 3 seeds here answer whether that result holds up.
...
Output:
  models/multiseed/gru_neopt_seed{N}_best.pt
  results/multiseed/gru_neopt_seed{N}_epochlog.csv
...
def train_one_seed(seed: int, train_dl, val_dl) -> dict:
    ckpt_path = MODEL_DIR / f"gru_neopt_seed{seed}_best.pt"
    if ckpt_path.exists():
        print(f"  [skip] gru_neopt seed={seed} — checkpoint already exists")
    ...
    # Phase 1: warm-up (single output, no logvar head)
    ...
    pd.DataFrame(epoch_log).to_csv(RES_DIR / f"gru_neopt_seed{seed}_epochlog.csv", index=False)
...
def main():
    print("=" * 60)
    print("MSA-NeOpt — GRU-NeOpt Multi-Seed Training")
    ...
    summary.to_csv(RES_DIR / "gru_neopt_multiseed_summary.csv", index=False)

    print("\n" + "=" * 60)
    print("GRU-NeOpt — mean +/- std across seeds")
    print("=" * 60)
    ...
    print(f"\nSaved: results/multiseed/gru_neopt_multiseed_summary.csv")

...
def load_gru_neopt():
    m = GRUBackbone().to(DEVICE)
    ckpt = torch.load(MODEL_DIR / "gru_neopt_best.pt", map_location=DEVICE)
    m.load_state_dict(ckpt["model_state_dict"])
    m.eval()
    return m
...
def get_predictions(model, loader, model_key: str):
    ...
    for X, y in loader:
        ...
        if model_key in ("neopt", "sspo"):   # [FIX-3] was "dbb"
            ...
            # PTO, GRU-NeOpt, MSABlock — all return single tensor
            mu = model(X)
        preds.append(mu.cpu())
        trues.append(y)
...
def plot_regret_boxplot(metrics_dict: dict):
    ...
    colours = ["#4C72B0", "#DD8452", "#55A868", "#8172B2", "#C44E52"]
...
def plot_learning_curves():
    configs = [
        ...
        ("sspo",      "SSPO Ablation",     "results/sspo_log.csv",      "val_regret"),
        ("gru_neopt", "GRU-NeOpt",         "results/gru_neopt_log.csv", "val_regret"),
...
def main():
    ...
    models = {
        ...
        "GRU-NeOpt":        (load_gru_neopt, "gru_neopt"),

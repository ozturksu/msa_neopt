...
Output:
  models/multiseed/sspo_seed{N}_best.pt
  results/multiseed/sspo_seed{N}_epochlog.csv
...
def train_one_seed(seed: int, train_dl, val_dl) -> dict:
    ckpt_path = MODEL_DIR / f"sspo_seed{seed}_best.pt"
    if ckpt_path.exists():
        print(f"  [skip] sspo seed={seed} — checkpoint already exists")
    ...
    # Phase 2: SSPO surrogate fine-tuning
    ...
    for epoch_ft in range(1, FINETUNE_EPOCHS + 1):
        ...
        with torch.no_grad():
            for X, y in val_dl:
                ...
                val_regret_total += regret.mean().item() * len(X)
                n += len(X)
        val_regret = val_regret_total / n
        scheduler.step(val_regret)

        e_time = time.time() - e0
        epoch_log.append({"phase": "sspo", "epoch": epoch,
                          "val_regret": val_regret, "epoch_seconds": e_time})
        if epoch_ft % 5 == 0 or epoch_ft == 1:
            print(f"    [sspo]   epoch {epoch:>3}  val_regret={val_regret:.6f}  ({e_time:.1f}s)")

        if val_regret < best_val_regret:
            best_val_regret, best_epoch, patience_counter = val_regret, epoch, 0
            torch.save({"epoch": epoch, "model_state_dict": model.state_dict(),
                       "val_regret": val_regret, "seed": seed}, ckpt_path)
        else:
            patience_counter += 1
    ...
    pd.DataFrame(epoch_log).to_csv(RES_DIR / f"sspo_seed{seed}_epochlog.csv", index=False)
...
def main():
    print("=" * 60)
    print("MSA-NeOpt — SSPO Multi-Seed Training")
    ...
    summary.to_csv(RES_DIR / "sspo_multiseed_summary.csv", index=False)

    print("\n" + "=" * 60)
    print("SSPO — mean +/- std across seeds")
    print("=" * 60)
    ...
    print(f"\nSaved: results/multiseed/sspo_multiseed_summary.csv")

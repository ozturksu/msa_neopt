#!/usr/bin/env python3
"""
train_neopt_multiseed.py
NeOpt (KimBackbone + SPO+) across 3 random seeds, own process.

Imports ONLY from step03_train_neopt.py (verified directly against your
real file, not a reconstruction, the safest of these five scripts).

Per supervisor feedback:
  - Full per-epoch timing + metrics logged, total training time recorded
  - Resumable: skips any seed whose checkpoint already exists

Output:
  models/multiseed/neopt_seed{N}_best.pt
  results/multiseed/neopt_seed{N}_epochlog.csv
...
def train_one_seed(seed: int, train_dl, val_dl) -> dict:
    ckpt_path = MODEL_DIR / f"neopt_seed{seed}_best.pt"
    if ckpt_path.exists():
        print(f"  [skip] neopt seed={seed} — checkpoint already exists")
    ...
    for epoch in range(1, WARMUP_EPOCHS + 1):
        ...
        with torch.no_grad():
            for X, y in val_dl:
                X, y = X.to(DEVICE), y.to(DEVICE)
                mu, _ = model(X)
                val_mse_total += criterion(mu, y).item() * len(X)
                val_mae_total += (mu - y).abs().mean().item() * len(X)
                mu_c = mu.clamp(0.0, 2.0)
                peak_hat = greedy_peak(mu_c)
    ...
    for epoch_ft in range(1, FINETUNE_EPOCHS + 1):
        ...
        for X, y in train_dl:
            ...
            loss = spo_plus_loss(mu_c, y, sol_true)
            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimiser.step()
        ...
        with torch.no_grad():
            for X, y in val_dl:
                X, y = X.to(DEVICE), y.to(DEVICE)
                mu, _ = model(X)
                val_mse_total += nn.functional.mse_loss(mu, y).item() * len(X)
                val_mae_total += (mu - y).abs().mean().item() * len(X)
                mu_c = mu.clamp(0.0, 2.0)
                peak_hat = greedy_peak(mu_c)
                peak_oracle = greedy_peak(y)
    ...
    pd.DataFrame(epoch_log).to_csv(RES_DIR / f"neopt_seed{seed}_epochlog.csv", index=False)
...
def evaluate_on_test(seed: int, test_dl) -> dict:
    ...
    for X, y in test_dl:
        X = X.to(DEVICE)
        mu, _ = model(X)
        preds.append(mu.cpu()); trues.append(y)
...
def main():
    print("=" * 60)
    print("MSA-NeOpt — NeOpt Multi-Seed Training")
    ...
    summary.to_csv(RES_DIR / "neopt_multiseed_summary.csv", index=False)

    print("\n" + "=" * 60)
    print("NeOpt — mean +/- std across seeds")
    print("=" * 60)
    ...
    print(f"\nSaved: results/multiseed/neopt_multiseed_summary.csv")
